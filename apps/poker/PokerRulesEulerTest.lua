...
for _,rank in pairs(Ranks) do
   abbv_to_rank[rank.abbv] = rank
end

local abbv_to_suit = {
}
for _,suit in pairs(Suits) do
   abbv_to_suit[suit.abbv] = suit
end

local function abbv_to_card(abbv)
   local rank_abbv = string.sub(abbv,1,1)
   local suit_abbv = string.sub(abbv,2,2)
   return Card(abbv_to_rank[rank_abbv], abbv_to_suit[suit_abbv])
end
...
for _,matchup in ipairs(matchups) do
   ...
   hand2={}
   for i=1,5 do
      hand1[i] = abbv_to_card(matchup[i])
   end
   for i=1,5 do
      hand2[i] = abbv_to_card(matchup[i+5])
   end
   if compare_hands(hand1,hand2) == -1 then
      wins = wins+1
   end
end
assert(wins==376)
