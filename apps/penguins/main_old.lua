	...
	if (event_list[levels[current_level]] ~= nil) then
		for key,value in pairs(event_list[levels[current_level]]) do
			if (not value["triggered"]) then
				...
				if (row == value["row"] and ms > value["time"]) then
					do_event(value,ms)
				end
			end
		end
...
function switch_levels:on_new_frame(ms,progress)
    groups[levels[current_level]].position = {1920*(1-progress),720*(1-progress)}
    groups[levels[current_level-1]].position = {-1920*progress,-720*progress}
end
...
function kill()
	...
	skating:advance_to_marker("spawn")
	update_reset()
...
function skating:on_new_frame(ms,progress)
    player.x = (2*(row % 2)-1) * progress * 1845 + ((row+1) % 2) * 1885
	--print(player.x, "\t", player.y)
    update(skating.elapsed)
end

function skating:on_started()
	--print("start \t",row)
	current_ground = ground[row]
	gravity = 0.1
    player.x = ((row+1) % 2) * 1920
    player.y = ground[row]
    up_speed = 0
    jumping = false
    --player.scale = {(2*(row % 2)-1),1}
