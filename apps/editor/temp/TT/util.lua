...
function itemTostring(v, d_list, t_list)
	...
	dumptable(d_list) 
	 src = v.source 
	 if not is_in_list(src.name, d_list) then 
	 	print("clone.source", src.name)
		print("clone's source is not in the d_list")
		print("clone's source is not in the d_list")
		print("clone's source is not in the d_list")
		print("clone's source is not in the d_list")
		...
		if v.extra.is_in_group ~= true then 
			...
			t_list = {v}
		        ...
		        table.insert(t_list, v) 
	             ...
	             print("todo Clone : ", v.name)
		     dumptable(t_list) 
		     print("done list ::::")
		     dumptable(d_list) 
		else 
		     print("this clone ", v.name, " is in group")  
		end 
	...
	for i,j in pairs (d_list) do 
		...
		itm_str, done_list, todo_list = itm_str..itemTostring(e, d_list, t_list)
		if itm_str ~= "" then 
		     d_list = done_list
		     t_list = todo_list
		     if i == 1 then
