...
local widget_f_map = {
     ...
     ["Image"]		= function () input_mode = S_SELECT editor.image() end, 	
...
,
	...
	local function make_msgw_item(caption) 
		local text = Text{ text = caption, reactive = true}:set( WSTYLE )
		local stext = Text{ text = caption, reactive = true}:set( WSSTYLE )
		return text, stext
	end 
...
function editor.rectangle(x, y)
	...
	while (is_available("rectangle"..tostring(item_num)) == false) do  
		item_num = item_num + 1
	end 
    ui.rect = Rectangle{
