...
local number_attr_t = {"bwidth", "hor_arrow_y", "vert_arrow_x", "cx", "cy", "cw", "ch", "bw", "bh", "bx", "by", "ix", "iy", 
	"left", "top", "width", "height", "x", "y", "z", "w", "h", "opacity", "button_radius", "select_radius", "line_space", "volume", "ui_width", "ui_height", "border_width", "border_corner_radius", "on_screen_duration", "fade_duration", "padding", "label_padding", "display_width", "display_height", "arrow_sz", "arrow_dist_to_frame", "visible_w", "visible_h", "virtual_w", "virtual_h", "frame_thickness", "bar_thickness", "bar_offset", "box_width", "overall_diameter", "dot_diameter", "number_of_dots", "cycle_time", "progress", "menu_width", "horz_padding", "vert_spacing", "horz_spacing", "vert_offset", "separator_thickness", "rows", "columns", "cell_w", "cell_h", "cell_spacing", "cell_timing", "cell_timing_offset", "title_separator_thickness", "selected_item", 
} 

local verify_attr_val = function(attr_name, attr_val, v, inspector) 
	
	local color_attr = string.find(attr_name, "color")
	local rotation_attr = string.find(attr_name, "angle") 
	local number_attr 

	for i, j in pairs (number_attr_t) do 
		if j == attr_name then 
			number_attr = true 
		end 
	end 
	...
	if number_attr or color_attr or rotation_attr then 
		if tonumber(attr_val) == nil then 
			editor.error_message("011",attr_name,nil,nil,inspector)
	...
	elseif attr_name == "selected_items" then 
		local items_tbl = {}
		local items_idx = 1
			...
			for i, j in pairs(item_group.children) do 
	      		if j.name then
					if item_group:find_child(j.name):find_child("input_text") then 
						...
						if verify_attr_val(j.name, itxt, v, inspector) then 
							return -1
						end 
					end
				end 
      		...
      		for i, j in pairs(item_group.children) do 
			 	...
			 	--	Boolean Text Input Field 
					...
					elseif j.name == "bx" or j.name == "by" then
						...
						if tonumber(itxt) then 
							if j.name == "bx" then bpos_t[1] = tonumber(itxt)
							elseif j.name == "by" then bpos_t[2] = tonumber(itxt)
							end 
