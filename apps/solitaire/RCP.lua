...
Controller = Class(Observer, function(ctrl, router, id)
    ...
    local ControlKeyTable = {
        [keys.Up] = function() ctrl:move_selector(Directions.UP) end,
        [keys.Down] = function() ctrl:move_selector(Directions.DOWN) end,
        [keys.Left] = function() ctrl:move_selector(Directions.LEFT) end,
        [keys.Right] = function() ctrl:move_selector(Directions.RIGHT) end,
        [keys.BackSpace] =
            function()
                if ctrl.back_pressed then ctrl:back_pressed() end 
            end,
        [keys.Return] =
            function()
                ctrl:run_callback()
    ...
    ControlKeyTable[keys.BACK] = ControlKeyTable[keys.BackSpace]
