HandState = Class(nil,function(state, ctrl, ...)
    ...
    -- index of active player in players table..
    -- it should be the case that players[players_action] == in_players[action]
    local players_action
    -- player => done table. if done[player] then player is in the hand,
    -- but doesn't need to bet anymore
    local done
    ...
    function state:get_bets_done()
        ...
        for _,player in ipairs(state:get_in_players()) do
            if player_bets[player] > all_in_bet then
                all_in_bet = player_bets[player]
            end
    ...
    function state:initialize(args)
        ...
        sb_qty = args.sb_qty or error("No small blind quantity", 2)
        bb_qty = args.bb_qty or error("No big blind quantity", 2)
        dealer = args.dealer or error("No dealer", 2)
        sb_p = args.sb_p or error("No small blind player", 2)
        bb_p = args.bb_p or error("No big blind player", 2)
        deck = args.deck or error("No deck", 2)
        round = Rounds.HOLE
    ...
    local function get_only_player()
        ...
        for _,player in ipairs(players) do
            if not out[player] then
                if only_player then
                print("get_only_player called with more than one player "
                    .."left... returning a remaining player")
            end
                only_player = player
            end
        end
    ...
    function state:execute_bet(fold, bet)
        ...
        or (call_bet < bet and bet < call_bet+min_raise
        and active_player.money==0) then
            delta = bet - player_bets[active_player]
            player_bets[active_player] = bet
            for _, player in ipairs(players) do
                if player ~= active_player and not out[player]
                and not all_in[player] and player.money > 0 then
                    done[player] = false
                end
            end
    ...
    -------------- player finished bet, figure out next step ----------------
        ...
        if get_num_inplayers() == 1 then
            ...
            only_player.money, player_bets[only_player], pot =
                only_player.money+player_bets[only_player]+pot, 0, 0
            ctrl:clear_pipeline()
        ...
        if continue then
            ...
            -- bet
            while out[players[tmp_action]] or done[players[tmp_action]]
            or all_in[players[tmp_action]] do
                if all_in[players[tmp_action]] then
                    print("player "..tmp_action.." is all in")
                    num_all_in = num_all_in+1
                end
    ...
    local function remove_player(i)
        local removed_player = table.remove(players, i)
    ...
    function state:showdown()
        ...
        result, tmp_poker_hand =
            compare_hands(in_hands[best], in_hands[in_players[1]])
        final_hands[in_players[1]] = in_hands[in_players[1]]

        -- compare all hands
        for i = 2,#in_players do
            result, tmp_poker_hand =
                compare_hands(in_hands[best], in_hands[in_players[i]])
