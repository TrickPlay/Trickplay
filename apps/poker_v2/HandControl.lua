if not HandState then dofile("HandState.lua") end
if not HandPresentation then dofile("HandPresentation.lua") end
...
HandControl = Class(nil,function(ctrl, game_ctrl, ...)
    ...
    local orig_hand_pipeline = {
        ...
        function(ctlr) return ctrl:showdown_animations() end,
        ...
        elseif not ctrl.waiting_for_bet then
            ...
            if active_player.is_human then
                ...
                if not fold then
                    ...
                    assert(
                        orig_bet <= bet,
                        "bet ($".. bet ..") should be at least as much "
                        .."as previous bet ($"..orig_bet..")."
                    )
                    assert(
                        bet <= active_player.money+orig_bet,
                        "bet ($".. bet ..") should max out at player's "
                        .."bank plus original bet ($"
                        ..active_player.money+orig_bet..")."
                    )
    ...
    function ctrl:showdown()
        local poker_hand
        winners, poker_hand = state:showdown()
    ...
    function ctrl:showdown_animations()
        pres:showdown_animations(winners)
