...
assets =
{
    ...
    enemy_missile   = Image{ src = "assets/bullet.png" },
    explosion1      = Image{ src = "assets/explosion1_strip8.png" },
    explosion2      = Image{ src = "assets/explosion2_strip6.png" },
...
ENEMY_PLANE_MIN_SPEED       = 150

ENEMY_PLANE_MAX_SPEED       = 200

ENEMY_FREQUENCY             = 0.8

ENEMY_SHOOTER_PERCENTAGE    = 100
...
my_plane =
{
    ...
    setup =
        ...
        function( self )
            ...
            local tw = self.image.w / 3
            local th = self.image.h
            
            self.group = Group{ size = { tw , th } , clip = { 0 , 0 , tw , th } }
    ...
    render =
        ...
        function( self , seconds )
            ...
            local x = math.floor( self.image.x - tw )
            
            if x <= - math.floor( self.image.w ) then
            ...
            if not self.dead then
                ...
                if self.v_speed > 0 then
                    ...
                    if y > screen.h - self.group.h then
                    
                        y = screen.h - self.group.h
                ...
                add_to_collision_list( self , start_point , self.group.center , { self.group.w , self.group.h } , TYPE_ENEMY_PLANE )
            ...
            
            {
                ...
                image =
                    ...
                    {                    
                        ...
                        position = { self.group.x + self.group.w / 2 , self.group.y - 14 }
                    },
    ...
    on_key =
        ...
        function( self , key )
            ...
            elseif key == keys.Return then
                ...
                if self.last_bullet.elapsed > 100 then
                
                    add_to_render_list( self:new_bullet() )
                    
                    self.last_bullet:start()
                        ...
                        self.group = Group
                            {
                                ...
                                children = { self.image },
                                z_rotation = { 180 , self.image.w / 3 / 2 , self.image.h / 2 }
                            }
                            ...
                            
                            {
                                ...
                                setup =
                                    ...
                                    function( self )
                                        ...
                                        self.group = Group
                                            {
                                                size = { self.image.w / 8 , self.image.h },
                                                position = enemy.group.center,
                                                clip = { 0 , 0 , self.image.w / 8 , self.image.h },
                                                children = { self.image },
                                                anchor_point = { ( self.image.w / 8 ) / 2 , self.image.h / 2 }
                                            }
                                ...
                                render =
                                    ...
                                    function( self , seconds )
                                        ...
                                        if self.time > self.duration then
                                            ...
                                            local frame = math.floor( self.time / ( self.duration / 8 ) )
                                            
                                            self.image.x = - ( ( self.image.w / 8 ) * frame )
...
function missile_test()
    
    local enemy = { speed = 150 , group = { center = { screen.w / 2 , 100 } } }
    
    local bullet =
    {
        speed = enemy.speed * 1.5,
        
        image = Clone{ source = assets.enemy_missile , opacity = 255 },
        ...
        setup =
            ...
            function( self )
                ...
                self.image.z_rotation = { 180 , self.image.w / 2 , self.image.h / 2 }
        ...
        render =
            ...
            function( self , seconds )
                ...
                local function angle_diff( a , b )
                    ...
                    local result = b - a
                    while result < -180 do result = result + 360 end
                    while result > 180 do result = result - 360 end
                    return result
                ...
                local mx , my = unpack( self.image.center )
                
                local px , py = unpack( my_plane.group.center )
                ...
                local da = angle_diff( angle , self.last_angle )
...
if false then
                ...
                self.image.z_rotation = { 90 + angle , self.image.w / 2 , self.image.h / 2 }
                
                local sq = math.sqrt( dx^2 + dy^2 )
                
                local speed = 200 * seconds
                
                local nx = speed * dx / sq
                
                local ny = speed * dy / sq
                
                self.image.position = { self.image.x + nx , self.image.y + ny }
                
if false then                
                
                local angle_change = math.acos((self.vector_save.x*dx + self.vector_save.y*dy)/(math.sqrt(dx^2+dy^2)*math.sqrt(self.vector_save.x^2+self.vector_save.y^2)))

--                print("angle change: ", self.vector_save.x, self.vector_save.y, dx/math.sqrt(dx^2+dy^2), dy/math.sqrt(dy^2+dx^2), math.deg(angle_change))

                if(math.abs(angle_change) > math.pi/2 * seconds) then
                    local adjust_angle = math.pi/2 * seconds * choose(angle_change>0,1,-1)
                    print("BOO")
                    dx = self.vector_save.x*cos(adjust_angle) - self.vector_save.y*sin(adjust_angle)
                    dy = self.vector_save.x*sin(adjust_angle) + self.vector_save.y*cos(adjust_angle)
                    print("angle too big", math.deg(angle_change), math.deg(adjust_angle), dx, dy)
                end
                ...
                local per = 200 * seconds
                
                local nx,ny = per * dx / math.sqrt( dx^2 + dy^2 ),
                                per * dy / math.sqrt( dx^2 + dy^2 )

                self.vector_save = { x = dx, y = dy }
