Model = Class(nil, -- base class
   function(model, ...) -- constructor, called with Model(...)
      ...
      local previous_component = nil

      -- class methods
      function model:attach(observer, controller_id)
         registry[observer] = true
         if controller_id then
            assert(not controllers[controller_id], "Controller with id " .. controller_id .. " already set!")
      ...
      function model:set_active_component(comp)
         print("Component: "..comp.." selected")
         previous_component = active_component
         active_component = comp
         print("Active Component", active_component, "set")
      end
   end)
