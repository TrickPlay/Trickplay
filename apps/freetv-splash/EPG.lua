...
local function reused_text(p)
    
    return (table.remove(recycled_text) or Text()):set(p)
    
end

local clear_row = function(r)
    local s = r.icon.scheduling
    if r.icon.no_prog then
        table.insert(recycled_text,r.icon.no_prog.actual)
        r.icon.no_prog.actual:unparent()
        return
    end
    for i=r.icon.left_i,r.icon.right_i do
        local show = s[i]
        if show.show_name and show.show_name.actual then
            table.insert(recycled_text,show.show_time.actual)
            table.insert(recycled_text,show.show_name.actual)
            show.show_time.actual:unparent()
            show.show_name.actual:unparent()
        end
    end
end
local populate_row = function(r)
    local s = r.icon.scheduling
    if r.icon.no_prog then
        r.icon.no_prog.actual = reused_text(r.icon.no_prog)
        r.icon.no_prog.actual.x = show_grid__left_edge + show_name_margin
        r.icon.show_names:add(r.icon.no_prog.actual)
        return
    end
    r.icon.left_i  = #s
    r.icon.right_i = 1
    ...
    for i,show in ipairs(s) do
        if i == 1 then print(s[1].x,s[#s].x) end
        if not (
            ...
            if show.show_name then
                show.show_time.actual = reused_text(show.show_time)
                show.show_name.actual = reused_text(show.show_name)
                r.icon.show_times:add( show.show_time.actual )
                r.icon.show_names:add( show.show_name.actual )
                
                show.show_name.actual:set{
                    --duration = dur,
                    x = show.x + show_name_margin,
                    w = show.w < (2*show_name_margin) and 0 or show.w - 2*show_name_margin,
                }
                show.show_time.actual:set{
                    --duration = dur,
                    x = show.x + show_name_margin,
                    w = show.w < (2*show_name_margin) and 0 or show.w - 2*show_name_margin,
                }
    ...
    dx = dx < 0 and 0 or dx
    if s[r.icon.left_i].x < show_grid__left_edge and s[r.icon.left_i].show_name and s[r.icon.left_i].show_name then
        s[r.icon.left_i].show_name.actual:set{
            --duration = dur,
            x = show_grid__left_edge + show_name_margin,
            w = s[r.icon.left_i].w - dx - 2*show_name_margin,
        }
        s[r.icon.left_i].show_time.actual:set{
            --duration = dur,
            x = show_grid__left_edge + show_name_margin,
            w = s[r.icon.left_i].w - dx - 2*show_name_margin,
        }
...
function instance:setup_icons(t)
    ...
    local item
    local i = 1
    for number,channel in pairs(t) do
        ...
        channel_list[i].number = number
        channel_list[i].id     = channel.id
        i = i + 1
    end
...
local function extract_time(s, file)
    ...
    if t.hour == "" then t.hour = 22 print(file,s) end
    if t.year == nil or t.day == nil or t.month == nil or t.hour == nil or t.min == nil then
    t.year,
    t.month,
    t.day,
    t.min = 
...
function instance:load_scheduling(t,old_time)
    --if old_time then 
        setup_curr_time(os.time{year=2013,month=1,day=18})--old_time) 
    --end
    --t = t.Channels.Channel
    
    scheduling = {}
    local slot, events, start_time, end_time
    for i,channel in pairs(channel_list) do
        
        events = readfile( "tv_guide_json/channel_"..channel.id..".json" )
        if events == nil then print("no file for tv_guide_json/channel_"..channel.id..".json") else
        events = json:parse( events )
        if events == nil then error("bad json for "..channel.id) end
        ...
        if events then
            for i, e in ipairs(events) do
                ...
                start_time = extract_time(e.start_time,"tv_guide_json/channel_"..channel.id..".json")
                --[[
                if i == 1 then 
                    print("difftime",os.difftime(os.time(start_time),os.time(curr_time)))
                ...
                end_time   = {
                    ...
                    min    = (start_time.min or 0) + e.duration,
                ...
                slot.w = e.duration/60 * 2*half_hour_len
