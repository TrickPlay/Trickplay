...
static void _TP_ImageDecoder_FreePixels(void *pixels)
{
	free(pixels);
}

// typedef int (*TPImageDecoder)(void *data, unsigned long int size, TPImage *image, void *user)
static int _TP_ImageDecoder_DecodeImage(void *pData, unsigned long int size, TPImage *pImage, void *pUser)
{
