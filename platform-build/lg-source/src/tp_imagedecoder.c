...
#else /* USE_HW_IMAGE_DECODER */

#include "tp_common.h"
#include "tp_imagedecoder.h"

#endif /* USE_HW_IMAGE_DECODER */


#ifdef USE_HW_IMAGE_DECODER

#ifndef LIBJPEG_SUPPORTS_MEMSRC
...
static BOOLEAN _ImageDecoder_GetVideoVirtualAddrBase(UINT32* pVideoVirtAddrBase)
{
	...
	DBG_PRINT_TP("Video memory info: physical addr base=%p, size=%lu MB",
			(void*)videoPhysAddrBase, (videoMemorySize / 1024 / 1024));
...
static int _ImageDecoder_DecodeImage(void* pData, unsigned long int size, TPImage* pImage, void* pUser)
{
	...
	GPU_PIXEL_FORMAT_T	pixelFormat = GPU_PIXEL_FORMAT_RGBA;

	/* When pImage is NULL, just returns whether Nexus can decode this image. */
	if (pImage == NULL)
		return ((imageFormat == GPU_IMAGE_FORMAT_MAX) ?
			TP_IMAGE_UNSUPPORTED_FORMAT : TP_IMAGE_SUPPORTED_FORMAT);
...
#ifdef _TP_DEBUG
	TP_TIMESTAMP_T start = TP_Util_GetTimestamp();
#endif

	/*----------------------------------------------------------------------------
	  [TODO] NEXUS_PixelFormat_eA8_eB8_G8_R8 actually generates RGBA format pixels.
	   (reverse byte-order) I should figure this out from Broadcom.
	 */
#if (PLATFORM_TYPE == BCM_PLATFORM)
	pixelFormat = GPU_PIXEL_FORMAT_ABGR;
#endif

	if (DDI_GPU_AllocSurface(imageInfo.rect.w, imageInfo.rect.h,
				pixelFormat, &_gSurface) != OK)
	{
...
#ifdef _TP_DEBUG
	TP_TIMESTAMP_T end = TP_Util_GetTimestamp();
	DBG_PRINT_TP("%.3f sec to decode image %dx%d",
			TP_Util_GetElapsedTime(start, end), imageInfo.rect.w, imageInfo.rect.h);
#endif
