...
SpriteSheet::SpriteSheet(gint w, gint h, const guchar *pixels, gchar *names[], gint *data, gint n, SpriteSheetFlags flags) {
  ...
  material = calloc(n, sizeof(CoglMaterial *));
  texture = calloc(n, sizeof(CoglHandle *));
  width = calloc(n, sizeof(gint));
  height = calloc(n, sizeof(gint));
  num_sprites = n;
...
void SpriteSheet:get_sprite(gchar *name, CoglMaterial **m, gint *w, gint *h) {
