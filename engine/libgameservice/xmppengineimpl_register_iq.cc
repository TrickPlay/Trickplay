...
class XmppIqEntry2 {
  XmppIqEntry2(const std::string & id, const std::string & to,
...
private:
  friend class XmppEngineImplRegister;
...
XmppReturnStatus
XmppEngineImplRegister::SendIq(const XmlElement * element, XmppIqHandler * iq_handler,
  XmppIqCookie* cookie) {
  ...
  XmppIqEntry2 * iq_entry = new XmppIqEntry2(id,
                                              element->Attr(QN_TO),
                                              this, iq_handler);
...
XmppReturnStatus
XmppEngineImplRegister::RemoveIqHandler(XmppIqCookie cookie,
    XmppIqHandler ** iq_handler) {

  std::vector<XmppIqEntry2*, std::allocator<XmppIqEntry2*> >::iterator pos;

  pos = std::find(iq_entries_->begin(),
                  iq_entries_->end(),
                  reinterpret_cast<XmppIqEntry2*>(cookie));

  if (pos == iq_entries_->end())
    return XMPP_RETURN_BADARGUMENT;

  XmppIqEntry2* entry = *pos;
...
void
XmppEngineImplRegister::DeleteIqCookies() {
  for (size_t i = 0; i < iq_entries_->size(); i += 1) {
    XmppIqEntry2 * iq_entry_ = (*iq_entries_)[i];
    (*iq_entries_)[i] = NULL;
    delete iq_entry_;
  }
  iq_entries_->clear();
...
XmppReturnStatus
XmppEngineImplRegister::SendStanzaError(const XmlElement * element_original,
...
bool
XmppEngineImplRegister::HandleIqResponse(const XmlElement * element) {
  ...
  for (std::vector<XmppIqEntry2 *>::iterator it = iq_entries_->begin();
       it != iq_entries_->end(); it += 1) {
    XmppIqEntry2 * iq_entry = *it;
