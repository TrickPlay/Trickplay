#ifndef _TXMPP_XMPPENGINEIMPLREGISTER_H_
#define _TXMPP_XMPPENGINEIMPLREGISTER_H_
...
class XmppEngineImplRegister : public XmppEngine {
public:
  XmppEngineImplRegister();
  virtual ~XmppEngineImplRegister();
  ...
  //! this jid is always empty as engine of registration has no login account associated with it
...
private:
  ...
  void SignalConnected(const Jid& fullJid);
  ...
  public:
    StanzaParseHandler(XmppEngineImplRegister * outer) : outer_(outer) {}
  ...
  private:
    XmppEngineImplRegister * const outer_;
  };

  class EnterExit {
   public:
    EnterExit(XmppEngineImplRegister* engine);
    ~EnterExit();
   private:
  ...
  scoped_ptr<XmppConnectTask> connect_task_;
  std::string lang_;
  ...
  typedef std::vector<XmppIqEntry2*> IqEntryVector;
...
#endif  // _TXMPP_XMPPENGINEIMPLREGISTER_H_
